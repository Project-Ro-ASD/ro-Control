...
bool DnfManager::enableRepo(const QString &repoUrl)
{
    ...
    const auto result = runner.runAsRoot(
        QStringLiteral("dnf"),
        { QStringLiteral("install"), QStringLiteral("-y"), repoUrl }
    );

    return result.success();
}

DnfManager::PackageInfo DnfManager::queryPackage(const QString &packageName) const
{
    CommandRunner runner;
