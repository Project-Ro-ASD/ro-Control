...
void RamMonitor::stop() { m_timer.stop(); }
...
void RamMonitor::poll() {
  ...
  while (!stream.atEnd()) {
    ...
    if (line.startsWith(QStringLiteral("MemTotal:")))
      memTotal =
          line.split(QLatin1Char(' '), Qt::SkipEmptyParts)[1].toLongLong();
  ...
  if (percent != m_usedPercent) {
    m_usedPercent = percent;
    emit usedPercentChanged();
  }
}
